...
.saved-btn .saved-content {
    position: relative;
    z-index: 2;
}

.saved-btn .saved-glow {
    position: absolute;
    top: 0;
    left: -60%;
    width: 200%;
...
<nav class="navbar navbar-expand-lg fixed-top px-0"
  ...
  <div class="container-fluid px-0 mx-0 d-flex flex-wrap align-items-center">
    ...
    <div class="d-flex align-items-center flex-wrap pl-0 ml-0">
        ...
        <div class="nav-marquee">
            <div class="nav-track">
                ...
                <!-- 🔁 REPEATED ITEMS (ALL FIXED) -->
                <!-- NOTE: margin-left:0 applied to ALL images -->
    ...
    <!-- RIGHT SIDE -->
    <div class="d-flex align-items-center gap-2 ml-auto mr-3">

        <?php if ($isAdmin): ?>
        <button onclick="window.location.href='filesaved.php'"
            class="saved-btn nav-text-uniform mr-2">
            <span class="saved-content">
                <i class="fas fa-folder-open mr-1"></i> Saved🍪
            </span>
            <span class="saved-glow"></span>
        </button>
        <?php endif; ?>

        <span class="nav-text-uniform" style="color:#8b949e;">
            Powered by: <b>Cybeat</b>
        </span>

        <a href="logout.php" 
           class="nav-text-uniform"
           style="
           color:#ff4d4d;
           font-weight:bold;
           padding:4px 10px;
